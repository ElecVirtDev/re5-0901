...
const validateIconKey = (key, title) => {
  ...
  if (!iconMap[key]) {
    console.error(
      `[IconMap] '${title}' 메뉴의 iconKey='${key}'는 iconMap에 매핑되지 않음`
    );
    return null;
  }
  return iconMap[key];
                  ...
                  {menu.path ? (
                    ...
                    <div>
                      ...
                      >
                        ...
                        <FontAwesomeIcon
                          icon={
                            openIndex === idx ? faChevronDown : faChevronRight
                          }
                      ...
                      {menu.children && openIndex === idx && (
                        <ul className="ml-4 mt-1 space-y-1 border-l border-gray-200 pl-2">
                          {menu.children.map((child, cidx) => {
                            const childIcon = validateIconKey(
                              child.icon,
                              child.title
                            );
                            return (
                              <li key={cidx}>
                                ...
                                >
                                  {childIcon && (
                                    <FontAwesomeIcon icon={childIcon} />
                                  )}
                                  {child.title}
                                </Link>
                              </li>
